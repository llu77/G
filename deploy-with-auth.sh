...
# LMM Finance Deployment Script with Authentication
# This script handles Cloudflare authentication and deployment

set -e

echo "🚀 Starting LMM Finance Deployment with Authentication..."
echo "======================================================="
...
if npx wrangler whoami &> /dev/null; then
    ...
    echo "Please authenticate with Cloudflare using one of these methods:"
    echo ""
    echo "1. Using API Token (Recommended):"
    echo "   export CLOUDFLARE_API_TOKEN=your_api_token_here"
    echo ""
    echo "2. Using Global API Key:"
    echo "   export CLOUDFLARE_API_KEY=your_api_key_here"
    echo "   export CLOUDFLARE_EMAIL=your_email@example.com"
    echo ""
    echo "3. Interactive login:"
    echo "   npx wrangler login"
    echo ""
    echo "For API token, create one at: https://dash.cloudflare.com/profile/api-tokens"
    echo "Required permissions: Cloudflare Workers, Account:Read, User:Read"
    echo ""
    
    # Check if API token is provided as environment variable
    if [ -n "$CLOUDFLARE_API_TOKEN" ]; then
        print_status "Using API token from environment variable..."
        export CLOUDFLARE_API_TOKEN
    elif [ -n "$CLOUDFLARE_API_KEY" ] && [ -n "$CLOUDFLARE_EMAIL" ]; then
        print_status "Using API key and email from environment variables..."
        export CLOUDFLARE_API_KEY
        export CLOUDFLARE_EMAIL
    else
        print_error "No authentication method found. Please set environment variables or run 'npx wrangler login'"
        echo ""
        echo "Example:"
        echo "CLOUDFLARE_API_TOKEN=your_token_here ./deploy-with-auth.sh"
        exit 1
    fi
fi
...
npx wrangler kv:namespace create "lmm-finance-cache" --env production || print_warning "KV namespace may already exist"

print_success "KV storage setup completed"
...
LMM Finance Production Deployment Log
====================================
Date: $(date)
Version: 1.0.0
...
echo ""
echo "🎉 LMM Finance deployed successfully!"
echo "===================================="
...
echo "- Database: Cloudflare D1 with 10 tables"
echo "- Storage: Cloudflare KV for caching"
...
echo ""
echo "🔧 Next Steps:"
echo "1. Configure production email service API keys"
...
echo ""
echo "📞 Support:"
echo "- Technical Support: support@lmm-finance.com"
echo "- Documentation: https://docs.lmm-finance.com"
echo "- Issues: https://github.com/lmm-finance/issues"
echo ""
